...
gulp.task('serve', 
  gulp.series(
    gulp.parallel('views', 'styles', 'scripts', 'jslint', 'fonts'),
    function serve() {
      browserSync({
        notify: false,
        port: 9000,
      ...
      gulp.watch('app/**/*.jade', gulp.series('views'));
...
gulp.task('wiredep', () => {
  const stream1 = gulp.src('app/assets/scss/*.scss')
    .pipe(wiredep({
      ignorePath: /^(\.\.\/)+/
    }))
    .pipe(gulp.dest('app/assets/css'));

  const stream2 = gulp.src('app/templates/layouts/*.jade')
    .pipe(wiredep({
      ignorePath: /^(\.\.\/)*\.\./
    }))
    .pipe(gulp.dest('app'));

  return Promise.all([stream1, stream2]);
});
